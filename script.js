...
// 显示图片预览（统一的预览逻辑）
function showImagePreview(imageBase64, fileName, fileSize) {
    currentImageBase64 = imageBase64;
    elements.previewImage.src = imageBase64;
    elements.fileInfo.textContent = `${fileName} (${formatFileSize(fileSize)})`;
            ...
            try {
                ...
                if (errorData.error) {
                    ...
                    switch (errorData.error.code) {
                        case 'INVALID_FORMAT':
                            errorMessage = '图片格式无效，请上传正确的图片文件';
                            break;
                        case 'UNSUPPORTED_FORMAT':
                            errorMessage = errorData.error.message || '不支持的图片格式';
                            break;
                        case 'IMAGE_TOO_LARGE':
                            errorMessage = errorData.error.message || '图片文件过大';
                            break;
                        case 'NO_IMAGE':
                            errorMessage = '未检测到图片数据，请重新上传';
                            break;
                        case 'INVALID_CONCURRENCY':
                            errorMessage = '配置错误，请联系管理员';
                            break;
                        case 'UNAUTHORIZED':
                            errorMessage = '访问令牌无效，请检查URL中的token参数';
                            break;
                        case 'NO_API_KEY':
                            errorMessage = 'API密钥未配置，请联系管理员';
                            break;
                        default:
                            errorMessage = errorData.error.message || '识别服务暂时不可用';
                    }
                }
            } catch (e) {
...
function handlePastedImage(blob) {
    ...
    const reader = new FileReader();
    reader.onload = (e) => {
        showImagePreview(e.target.result, '粘贴的图片', blob.size);
    };
